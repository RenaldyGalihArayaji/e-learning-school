...
@section('content')
    <div class="title">
        <h1 class="h3 mb-0 text-gray-800">Master | <span class="text-secondary fs-5">Tahun Ajaran</span></h1>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function create() {
            $.get("{{ route('tahun-ajaran.create') }}", {}, function(data, status) {
        ...
        function store() {
            ...
            var dataToForm = $("#form-tahun-ajaran").serialize();
        ...
        // Tampilan Modal Eidt
        function edit(id) {
            $.get("{{ url('master/tahun-ajaran') }}/" + id + "/edit", {}, function(data, status) {
                $("#staticBackdropLabel").html('Edit Tahun Ajaran');
                $("#page").html(data);
                $("#staticBackdrop").modal("show");
            });
        }
        ...
        function update(id) {
            ...
            $.ajax({
                type: "PUT",
                url: "{{ url('master/tahun-ajaran') }}/" + id,
                data: dataToForm,
                success: function(response) {
                    if (response.status === 200) {
                        $(".btn-close").click();
